...
public class DataUploadHandler {
    ...
    private static boolean handleDocumentAnnotationSingleFileInternal(DReqObj reqObj, DProjects project, String filePath) {
        ...
        try {
            ...
            if(createHitWithContent(reqObj, project, content, false, false,null) != null) {
                status = true;
            }
        }
        catch (Exception e) {
    ...
    private static String createHitWithContent(DReqObj reqObj, DProjects project, String content, boolean hitStatus, boolean isURL,Long studyId) {

        if (Validations.isValidDataItemForTextTask(content, reqObj)) {
            DHits hits;
            if(studyId==null)
            hits= new DHits(project.getId(), content);
            else
            hits= new DHits(project.getId(), content,studyId);	
    ...
    public static UploadResponse handleImageClassification(DReqObj reqObj, DProjects project, String filePath) {
        return handleImageTasks(reqObj, project, filePath,null);
    }
    ...
    public static UploadResponse handleImagePolygonBoundingBox(DReqObj reqObj, DProjects project, String filePath,Long studyId) {
        return handleImageTasks(reqObj, project, filePath,studyId);
    }
    ...
    private static UploadResponse handleTasksWithTextFileWithURLs(DReqObj reqObj, DProjects project, String filePath) {
        ...
        if (type == DTypes.File_Type.TEXT) {
            ...
            return handleTasksWithURLs(reqObj, project, urls,null);
        }
        else {
    ...
    private static UploadResponse handleTasksWithURLs(DReqObj reqObj, DProjects project, List<String> urls,Long studyId) {
        UploadResponse response = new UploadResponse();
        ...
        if (urls != null && !urls.isEmpty()) {
            for (String url : urls) {
                ...
                try {
                    if (createHitWithContent(reqObj, project, url, false, true,studyId) != null) {
                        response.incrementHits();
                    }
                    else {
                        response.incrementIgnored();
    ...
    // the passed file is a zip or tar.
    private static UploadResponse handleImageTasksZip(DReqObj reqObj, DProjects project, String filePath,long studyId) {
        if (!DBBasedConfigs.isImageUploadAllowed()) {
            throw new WebApplicationException("Please upload a txt file containing only URLs in each line.", Response.Status.BAD_REQUEST);
        }
        ...
        try {
            ...
            UploadResponse response =  handleTasksWithURLs(reqObj, project, urls,studyId);
    ...
    private static UploadResponse handleImageTasks(DReqObj reqObj, DProjects project, String filePath,Long studyId) {
        ...
        if (type == DTypes.File_Type.IMAGE) {
            ...
            return handleImageTasksZip(reqObj, project, filePath,studyId);
        }
