...
import bonsai.email.MailService;
...
public class DataturksEndpoint {
    ...
    public UploadResponse fileUpload(@NotNull @HeaderParam("token") String token,
        ...
        try {
            ...
            UploadResponse response = handleFileUpload(reqObj, projectId, stream, fileDetail,null);
    ...
    //Upload dicom image
     @POST
    @Path("/{projectId}/upload/dicom/{studyName}")
    @Consumes(MediaType.MULTIPART_FORM_DATA)
    @Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
    public UploadResponse dicomfileUpload(@NotNull @HeaderParam("token") String token,
                             @NotNull @HeaderParam("uid") String id,
                             @NotNull @PathParam("projectId") String projectId,
                             @NotNull @PathParam("studyName") String studyName,
                             @NotNull @FormDataParam("file") InputStream stream,
                                     @HeaderParam("format") String uploadFormat,
                                     @HeaderParam("itemStatus") String itemStatus,
                             @FormDataParam("file") FormDataContentDisposition fileDetail) {
        ...
        try {
            ...
            Long studyId=studyHandler.saveStudy(studyName, projectId);
            UploadResponse response = handleFileUpload(reqObj, projectId, stream, fileDetail,null);
...
//    }
    ...
    public GetHits getHits(@NotNull @HeaderParam("token") String token,
        ...
        String regStr = "getHits: project= " +  projectId + 
                " status = " + status + "label = " + label + " evaluation=" + evaluation + " userId= " + userId +  " uid = " + id;
        LOG.info(regStr);
    ...
    public static UploadResponse handleFileUpload(DReqObj reqObj, String projectId, InputStream stream, FormDataContentDisposition fileDetail,Long studyId) {
        ...
        try {
            ...
            response =  Controlcenter.handleFileUpload(reqObj, projectId, uploadedFile,studyId);
            return response;
        }
        finally {
